...
async def cmd_start(message: types.Message) -> types.Message:
    if not message.from_user.username:
        await message.answer(f'''Привет, {message.from_user.full_name}!\nУ тебя не установлен username, установи его в настройках телеграм и напиши /start\n\
''', parse_mode='Markdown')
        return
...
async def cmd_pay(message: types.Message, state: FSMContext) -> types.Message:
    ...
    await bot.send_message(message.from_user.id, f'Для оплаты подписки переведите 100₽ на карту `{PAYMENT_CARD}` и отправьте скриншот чека/операции в ответ на это сообщение.', parse_mode='Markdown', reply_markup=await kb.cancel_payment_kb())
...
async def device_selected(call: types.CallbackQuery, state=FSMContext):
    """
    Этот обработчик будет вызываться, когда пользователь нажимает кнопку «ПК» или «Смартфон»
    """
    ...
    with open(f'data/temp/VPN_{call.from_user.username}_{device}.conf', 'w') as f:
        f.write(user_config)

    # send config file
    await call.message.answer_document(types.InputFile(f'data/temp/VPN_{call.from_user.username}_{device}.conf'),
                                       reply_markup=await kb.configs_kb(call.from_user.id))

    if device == "PHONE":
        # send qr code (create qr code from config by qrencode)
        os.system(f'qrencode -o data/temp/VPN_{call.from_user.username}.png -s 10 -l H -m 2 < data/temp/VPN_{call.from_user.username}_{device}.conf')
        await call.message.answer_photo(types.InputFile(f'data/temp/VPN_{call.from_user.username}.png'),)

    # delete temp files
    try:
        remove(f'data/temp/VPN_{call.from_user.username}.conf')
        if device == "PHONE":
            remove(f'data/temp/VPN_{call.from_user.username}.png')
...
async def cmd_show_config(message: types.Message, state=FSMContext):
    ...
    if device == 'PC':
        with open(f'data/temp/VPN_{message.from_user.username}_{device}.conf', 'w') as f:
            f.write(config)

        # send config file
        await message.answer_document(types.InputFile(f'data/temp/VPN_{message.from_user.username}_{device}.conf'),)

        # delete temp files
        try:
            remove(
                f'data/temp/VPN_{message.from_user.username}_{device}.conf')
        except OSError as error:
            logger.error(
        ...
        # send qr code (create qr code from config by qrencode)
        os.system(f'qrencode -o data/temp/VPN_{message.from_user.username}.png -s 10 -l H -m 2 < data/temp/VPN_{message.from_user.username}_{device}.conf')
        await message.answer_photo(types.InputFile(f'data/temp/VPN_{message.from_user.username}.png'),)

        # delete temp files
        try:
            remove(
                f'data/temp/VPN_{message.from_user.username}_{device}.conf')
            remove(f'data/temp/VPN_{message.from_user.username}.png')
        except OSError as error:
            logger.error(
                f'Error while deleting temp files for user {message.from_user.username}; Error: {error}')
...
async def cmd_support(message: types.Message):
    ...
    # answer with username info @frostreader as markdown
    await message.answer(
        'Если у тебя все еще остались вопросы, то ты можешь написать [мне](t.me/frostreader) лично',
        parse_mode='Markdown')
